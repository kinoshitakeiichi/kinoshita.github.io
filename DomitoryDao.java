...
public class DomitoryDao {	
	...
	public List<DomitoryBean> GetList(String strwhere,String strorder){
		...
		try{
			...
			while(rs.next()){
				...
				cnbean.setDomitory_Number(rs.getInt("Domitory_Number"));
	...
	//添加人数
	public void AddN(int id,int N){
		
		String sql="update Domitory set ";
		sql+="Domitory_Number='"+(GetBean(id).getDomitory_Number()+N)+"'";
		
		sql+=" where Domitory_ID='"+id+"'";
		Statement stat = null;
		ResultSet rs = null;
		Connection conn = new DBHelper().getConn();
		try{
